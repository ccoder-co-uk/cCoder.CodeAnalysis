...
| RFC0002 | A CRUD deletion returns `202 Accepted` when processing continues asynchronously, or `204 No Content` when deletion completes synchronously. | OData controller CRUD `DELETE`. | Every successful return path is `Accepted()`/202 or `NoContent()`/204 and supplies no content. | Delete operations intentionally returning a representation under a separately adopted contract. | Yes. | RFC 9110 [§9.3.5](https://www.rfc-editor.org/rfc/rfc9110.html#section-9.3.5), [§15.3.3](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.3.3), [§15.3.5](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.3.5) |
...
| RFC0011 | A `204 No Content` response has no response body. | Any modeled HTTP response explicitly returns status 204. | The response path supplies no content value. | Response metadata headers, which remain permitted. | Yes: response-body presence is modeled. | RFC 9110 [§15.3.5](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.3.5) |
| RFC0012 | A response to `HEAD` has no response body. | Any HTTP exposure handles HEAD. | Every modeled response path supplies no content value. | Representation metadata headers corresponding to GET. | Yes. | RFC 9110 [§9.3.2](https://www.rfc-editor.org/rfc/rfc9110.html#section-9.3.2) |
| RFC0013 | Unsupported request media maps to `415 Unsupported Media Type`. | An HTTP exposure has an escaping exception classified, transitionally by name, as `UnsupportedMedia`. | A reachable catch maps the exception to 415. | Unsupported response media and malformed payloads requiring a different status. | Yes after catch/response queries. | RFC 9110 [§15.5.16](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.5.16) |
| RFC0014 | A failed request precondition maps to `412 Precondition Failed`. | An HTTP exposure has an escaping exception classified, transitionally by name, as `Precondition` or `ETag`. | A reachable catch maps the exception to 412. | Missing required preconditions and non-precondition state conflicts. | Yes after catch/response queries. | RFC 9110 [§15.5.13](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.5.13) |
| RFC0015 | An explicitly returned HTTP status is a valid three-digit status code. | A modeled response supplies a numeric status code. | The value is between 100 and 599 inclusive. | Framework results whose eventual status is selected at runtime and cannot be resolved statically. | Yes. | RFC 9110 [§15](https://www.rfc-editor.org/rfc/rfc9110.html#section-15) |
...
1. Add authentication-challenge evidence to strengthen RFC0006 beyond its current status-code check.
2. Replace transitional exception-name classification with explicit exception categories while preserving names as a compatibility fallback.
3. Add OData action/function and optimistic-concurrency metadata; implement ODATA0005 and ODATA0006. Keep ODATA0004 as an audit until ordered mutation facts exist.
